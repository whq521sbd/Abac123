...
public class DealFragment extends Fragment implements OnClickListener {
    ...
    * */
    ...
    private void initUIData(String sWeight, String sTotal) {
        ...
        if (!StringUtils.isEmpty(myGoodsA.getPic().split(",")[0])) {
            gPic = myGoodsA.getPic().split(",")[0];
        }
        ...
        if (myGoodsA != null) {
            ...
            FinalBitmap.create(getActivity()).display(goodsImg, FinalContent.FinalUrl + myGoodsA.getPic().split(",")[0]);
        } else {
                    ...
                    if (msg.arg1 > 0) {
                        ...
                        if (otd != null) {
                            ...
                            orderStr = createCodeImg(myGoodsA.getClassXXXID(),
                                    ...
                                    CommonTools.getDeviceInfo(getActivity()),
                                    totalPrice, weight,
                                    myGoodsA.getPic().split(",")[0], myGoodsA.getQuantity(), otd.getTransID()/*,
                                    buyNum,= editFreight.getText().toString()*/);
